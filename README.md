...
- 🤖 **Telegram бот** с командами управления (/status, /balances, /profit)
...
Для получения уведомлений о сделках в Telegram:

1. **Создайте бота** через [@BotFather](https://t.me/BotFather)
2. **Получите токен бота** и ваш Chat ID
3. **Добавьте настройки** в `.env` файл (см. выше)

📖 **Подробная инструкция**: [TELEGRAM_SETUP.md](TELEGRAM_SETUP.md)

🧪 **Тестирование уведомлений**: `python test_telegram.py`

### Telegram команды (опционально)

Управляйте ботом через Telegram команды:

- `/status` - статус бота и основная информация  
- `/balances` - текущие балансы по всем монетам
- `/profit` - детальная статистика прибыли
- `/summary` - краткая сводка по сделкам

📖 **Подробная инструкция**: [TELEGRAM_COMMANDS_SETUP.md](TELEGRAM_COMMANDS_SETUP.md)

🧪 **Тестирование команд**: `python test_telegram_commands.py`
...
### GET /balances  
Получить текущие балансы по всем монетам.

### GET /profit
Получить детальную статистику прибыли и убытков.
